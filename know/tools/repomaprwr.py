...
# ---------------------------------------------------------------------
class RepoMapRwr(ProjectComponent):
    component_name = "repomaprwr"
...
class RepoMapRWRTool(BaseTool):
    ...
    by the `RepoMapRwr` component.
    """
    tool_name = "vectorops_repomap_rwr"

    def __init__(self, *a, **kw):
        from know.project import Project
        Project.register_component(RepoMapRwr)   # one-time registration
        super().__init__(*a, **kw)
    ...
    def execute(
        ...
        *,
        ...
        repomap: RepoMapRwr = project.get_component("repomaprwr")  # type: ignore
        if repomap is None:
            raise RuntimeError(
                "RepoMapRwr component is missing.  Call "
                "`project.get_component('repomaprwr')` (or ensure it is "
        ...
        for path, score in ranked:
            ...
            results.append(
                RepoMapRWRScore(file_path=path, score=score, summary=summary)
            )

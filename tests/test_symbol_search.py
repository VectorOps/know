...
def test_bm25_embedding_search_20cases(data_repo, emb_calc):
    ...
    res_bm25 = sym_repo.search(NodeSearchQuery(repo_ids=[rid], doc_needle="sort", limit=5))
    ...
    res_date = sym_repo.search(NodeSearchQuery(repo_ids=[rid], doc_needle="date", limit=3))
    assert all("Date" in s.name for s in res_date), f"Top date results: {[s.name for s in res_date]}"

    # Query for 'HTTP'; network symbols
    res_net = sym_repo.search(NodeSearchQuery(repo_ids=[rid], doc_needle="HTTP", limit=2))
    ...
    emb_sort = sym_repo.search(NodeSearchQuery(repo_ids=[rid], embedding_query=sort_vec, limit=5))
    ...
    emb_math = sym_repo.search(NodeSearchQuery(repo_ids=[rid], embedding_query=math_vec, limit=5))
    ...
    comb = sym_repo.search(
        NodeSearchQuery(repo_ids=[rid], doc_needle="sort", embedding_query=sort_vec, limit=5)
    )
    ...
    assert sym_repo.search(NodeSearchQuery(repo_ids=[rid], limit=25))  # total exists

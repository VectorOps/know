...
def test_repomap_tool_pagerank_and_boost():
    ...
    res_boost_json = tool.execute(project, {"file_paths": [d_path]})
    ...
    res_boost_json = tool.execute(project, {"file_paths": [b_path]})
    res_boost = [RepoMapScore.model_validate(obj) for obj in json.loads(res_boost_json)]
    ...
    res_sym_boost_json = tool.execute(project, {"symbol_names": ["beta"]})

...
- Project links multiple repos together
- Move project_path out of settings
- Change Project API to return methods to add/remove repos to project
- Create data storage class
- Refresh repo should be a separate method and take path/repo id
- There should be a default repo for the project, passed to Project initialization
- All tools should be project aware - request only relevant repo_ids in the project (or default)
- Update all filters and search criteria to do repo_id IN (...) when filtering by repo
- Extend tools to search globally or by project
* [ ] Add Turso DB support
...
** [ ] Make tool search all symbols, not just public by default

...
** [ ] Create in-memory NetworkX graph similar to Aider?
- Add all project public symbols and all references
- Implement ranking algorithm to find recommendations
  - Symbols names that are defined more than X times are deprioritized
  - Symbols that are referenced in search query are boosted
  - Symbols in files that are requested are boosted
  - Descriptive names are prioritized
  - Private symbols are deprioritized
** [ ] Figure out incremental graph updates
** [ ] Create a tool that takes in file paths, query, runs above algorithm
